...
double VectorDot(const double* u, const double* v)
{
	return u[0] * v[0] + u[1] * v[1] + u[2] * v[2];
}

double Dot(const CPoint3D& pt1, const CPoint3D& pt2)
{
	return pt1.x * pt2.x + pt1.y * pt2.y + pt1.z * pt2.z;
}
void VectorCross(const double* u, const double* v, double* n)
{
	n[0] = u[1] * v[2] - u[2] * v[1];
	n[1] = u[2] * v[0] - u[0] * v[2];
	n[2] = u[0] * v[1] - u[1] * v[0];
...
double AngleBetween(const double* u, const double* v)
{
	double lenU = sqrt(u[0] * u[0] + u[1] * u[1] + u[2] * u[2]);
	double lenV = sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
	double dot = VectorDot(u, v) / lenU / lenV;
	if (dot < -1)
		dot = -1.0;
	if (dot > 1)
		dot = 1.0;	
